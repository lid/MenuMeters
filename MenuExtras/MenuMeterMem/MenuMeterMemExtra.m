...
- (void)renderPieIntoImage:(NSImage *)image {
	...
	// Load current stats
	float totalMB = 1.0f, activeMB = 0, inactiveMB = 0, wireMB = 0;
	NSDictionary *currentMemStats = [memHistory objectAtIndex:0];
	if (currentMemStats) {
...
- (void)renderBarIntoImage:(NSImage *)image {
	...
	NSBezierPath *inactivePath = [NSBezierPath bezierPathWithRect:NSMakeRect(1.5f, 1.5f, kMemThermometerDisplayWidth - 3, 
																		   thermometerTotalHeight * ((wireMB + activeMB + inactiveMB) / totalMB))];
	...
	float renderHeight = (float)[image size].height;
 	for (renderPosition = 0; renderPosition < [ourPrefs memGraphLength]; renderPosition++) {
		...
		[inactivePath lineToPoint:NSMakePoint(renderPosition, 
											  (inactiveMB + activeMB + wireMB) > totalMB ? totalMB : ((inactiveMB + activeMB + wireMB) / totalMB) * renderHeight)];
...
- (void)renderPageIndicatorIntoImage:(NSImage *)image {
	...
	NSAttributedString *renderString = [[[NSAttributedString alloc] 
											initWithString:countString
												attributes:[NSDictionary dictionaryWithObjectsAndKeys:
																[NSFont systemFontOfSize:9.5f], NSFontAttributeName,
																pageRateColor, NSForegroundColorAttributeName,
																nil]] autorelease];
	// Using NSParagraphStyle to right align clipped weird, so do it manually
...
- (void)configFromPrefs:(NSNotification *)notification {
	...
	// Handle ShapeShifter themes using Carbon API, looking for a workaround 
	// on x86_64 Cocoa
	[fgMenuThemeColor release];
	fgMenuThemeColor = nil;
#ifndef __x86_64__
	RGBColor rgbThemeColor;
	if (GetThemeTextColor(kThemeTextColorRootMenuActive, 24, true, &rgbThemeColor) == noErr) {
		fgMenuThemeColor = [[NSColor colorWithCalibratedRed:((float)rgbThemeColor.red / (float)0xFFFF)
													  green:((float)rgbThemeColor.green / (float)0xFFFF)
													   blue:((float)rgbThemeColor.blue / (float)0xFFFF)
													  alpha:(float)1.0] retain];
	} else {
		fgMenuThemeColor = [[NSColor blackColor] retain];
	}
#else
	// Assume black
	fgMenuThemeColor = [[NSColor blackColor] retain];
	...
	[pageRateColor release];
	pageRateColor = [[ourPrefs memPageRateColor] retain];
